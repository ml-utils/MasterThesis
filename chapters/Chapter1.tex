...
We find that, as in the results for Italian models, the GPT-2 architecture seems to be favoured in adjunct islads, in which it outperforms by at least 5 percentage points the BERT-based models.

..\\
% (show results also with PenLP, show that, contrary to Salazar and Lau et al, this also hurts the performance of GPT-2)
Our results confirm the findings by ..Salazar that the penalty term is beneficial only for unidirectional models (like GPT-2) and not for bidirectional ones (like BERT), because of their respective numerical properties of ..
Salazar observed/found that for GPT-2-like model, the penalty term actually has a dampening effect on the ..
In the sentence proability score given by GPT-2 models there is a confound/the overlapping of two phenomena: .. the decreasing of the curve at the advancing of the token position, and the spikes of surpraisals at individual token position.
