...
% todo: subsections for each of the four island types: e.g. for adjuncts, with example of conditional and temporan adjuncts; the fact that they were first studied in 1982
...
We can see that the plots for whether, adjunct, and subject islands from the GPT2-2 model (\autoref{fig:md_gpt_penlp}) are quite similar with those obtained on human subjects (\autoref{fig:sprouse_humans}), with the difference that for adjunct islands, the SHORT-ISLAND sentence gets a higher acceptability score from the transformer model (this might be due to the confound deriving from the format of this sentence, as discussed in section ..).


The complex np islands plots from the transformers models are instead quite different from those on humans. This might be due to ..

Comparing the plots from BERT with those on humans, we can see again the discrepancy with the SHORT ISLAND sentence of the adjunct island (likely due to the confound discussed above).

Bert seems to score the complex np islands better than GPT2, with plots that resemble more those from humans.

For subject island (that have ..a modified factorial design as discussed in section ..) we see that BERT scores higher the short island sentence than the short non island, contrary to what happens with humans and with GPT2. To better explain why we should have a look at the spike analysis in the following section. In any case, this effect seems to be subtracted out by the factorial score, so BERT still gets the best accuracy score for this type of island as seen in table ..
