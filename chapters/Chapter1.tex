...
With this qualitative analysis we try to explain the results in \autoref{tab:accuracy_DD_it_data}. Specifically, the fact that (1) for adjunct islands, there is a 6+ points drop in accuracy by BERT XXL (the model trained on the largest amount of data), compared to BERT and GilBERTo. That (2) for subject islands, there is a progressive increase in accuracy, which more or less correlates with amount of training data, from BERT (2B tokens), to BERT XXL (13B tokens), to GilBERTo (11B tokens).
...
Back to the question of why BERT is the only model not to reach 100\% accuracy on whether islands, there seems to be no clear explanation for this fact looking at the token surprisals of the same item across all the models, as shown in \autoref{fig:md_bert2b_whether_item_180_surprisals}-\ref{fig:md_gpt_whether_item_135_surprisals}.
One thing we notice is that the baseline acceptable sentence (\textsc{short-nonisland}) ("\textit{Chi pensa che io abbia aumentato l'affitto?}") ("\textit{Who thinks that I have raised the rent?}") gets a lower acceptability in BERT XXL (\autoref{fig:md_bertxxl_whether_item_156_surprisals}) than BERT (\autoref{fig:md_bert2b_whether_item_180_surprisals}). The trend of the surprisal plots seems to be almost the same between BERT and BERT XXL, with a few difference. We can notice the strange phenomenon that BERT XXL gets a high surprisal (much higher than BERT) on the words "\textit{affitto}" ("\textit{rent}") and "\textit{io}" ("\textit{I}"). Again this seems another case of semantic phenomena having a prevailing confounding effect, that could mask the effect of syntactic ones.
...
We can complement our analysis so far, with a comparison of the token surprisals for the same sentences across the four models we tested. In \autoref{fig:md_bert2b_adjunct_item_145_surprisals}, \ref{fig:md_bertxxl_adjunct_item_188_surprisals}, \ref{fig:md_gilberto_adjunct_item_185_surprisals}, \ref{fig:md_gpt_adjunct_item_163_surprisals} we can see token surprisals plots from BERT, BERT XXL, GilBERTo and GePpeTto on an adjunct island item. 
...
Back to the question of why BERT XXL achieves 100\% accuracy in complex NP islands, going 4 points above BERT, we can have a look at the token surprisals plots in \autoref{fig:md_bertxxl_complex_np_item_32_surprisals} and \autoref{fig:md_bert2b_complex_np_item_173_surprisals} for a sentence that BERT XXL scores correctly (DD score = 17.75) and BERT incorrectly (-0.21). 
...
Back to the question of explaining the correlation between the amount of model training data and the performance of these models on subject islands, we can have a look at the token surprisal plots for the same test item across the four models (\autoref{fig:md_bert2b_subject_item_185_surprisals}-\ref{fig:md_gpt_subject_item_171_surprisals}). In \autoref{fig:md_bertxxl_subject_item_167_surprisals}, we see that BERT XXL gives a better acceptability to the baseline sentence than the BERT model (\autoref{fig:md_bertxxl_subject_item_167_surprisals}), but this seems to be due again to lexical reasons, because BERT XXL gives much smaller surprisals to the words "\textit{foto}" ("\textit{photo}") and to the OOV word "\textit{scandalizzato}" ("\textit{scandalized}"). We also observe that in the two bottom sentences ("\textit{Di chi pensi che la foto dell'artista abbia suscitato l'indignazione?}", "\textit{Di chi pensi che la foto abbia suscitato l'indignazione del pubblico?}") ("\textit{of whom do you think that the artist picture caused the outrage?}", "\textit{of whom do you think that the picture cause the outrage of the audience?}") we notice that BERT, compared to BERT XXL, gives very high surprisal to an  apostrophe punctuation marking the determiner elision and followed by a OOV word ("\textit{l'indignazione}") ("\textit{the outrage}").\\
...
We can see the degrading effect in performance, due to normalizing by sentence length, comparing the plots for BERT XXL without normalization (\autoref{fig:md_bert_lp}) and with normalization (%\ref{A-fig:md_bert_penlp} 
