...
Developing a revised test suites to control for more confounds.
Split the test suite for adjunct islands into temporal adjuncts and conditional adjuncts.
Strictly balance the sentence in each item to use the same lexical content.
Control for the increased processing complexity due to interrogative forms. Duplicate the test suites with versions in the affirmative, as in \citet{wilcox2018rnn}.
Translate into Italian the Filler-Gap and island effects items from \citet{wilcox2018rnn}, and try their approach of assessing island effects as a reduction in expected gaps in the presence of a filler, and compare their approach with the one by \citet{sprouse2016experimental} adopted in this thesis.

Translate and adapt our test suites also into English, and compare the results with those from English speakers collected by \citet{sprouse2016experimental}, and compare also with the results on English island effects on the BLiMP test suites, and those from \cite{wilcox2018rnn}.

Control for OOV words, possibly creating a duplicate for each test suite, one with and one without OOV words.
Controlling for less common verb forms, e.g. using the indicative mood instead of the subjunctive mood.
Controlling for the effects of using personal pronouns and proper names (e.g. \textit{Gianni, Luisa}) instead of common nouns (e.g. lo studente, il dottore, etc.).
Controlling for the effect of using, within a sentence, words with strong association (e.g. \textit{lo studente legge il libro}, \textit{il poliziotto arresta il ladro}, etc.), vs marginal ones.
Test for also across rc-dependencies, as in \citep{sprouse2016experimental}
