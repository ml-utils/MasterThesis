...
\begin{figure}[p]
	...
	\caption{Plots of average scores from humans (\ref{fig:sprouse_humans}) taken from \citep{sprouse2016experimental}.
	}
...
\begin{figure}[p]
	\centering
	\ULsubfloat[Fig.\ref{fig:md_gpt_penlp} \label{fig:md_gpt_penlp}]{%
	...
	\caption{Plots of average scores from GePpeTto (\ref{fig:md_gpt_penlp})  and from GilBERTo (\ref{fig:md_gilberto_lp}), on the test suites developed for the present thesis. 
	}
	\label{fig:plots_3_mdd}
	% \small (..)
\end{figure}
...
\begin{figure}[p]
	...
	\vspace{10pt}
	\ULsubfloat[Fig.\ref{fig:md_bert2b_lp}  \label{fig:md_bert2b_lp}]{%
		\includegraphics[width=.9\textwidth]{images/Chapter1/Madeddu_wh_dbmdz_bert-base-italian-cased_LP-zscores-likert-2022-09-17_h11m10s08.png}%
	}
	\caption{Plots of average scores from BERT XXL \ref{fig:md_bert_lp} and BERT (\ref{fig:md_bert2b_lp}), on the test suites developed for the present thesis. 
	}
...
We see that, for complex NP items (top-right plot of each figure from \autoref{fig:sprouse_humans} to \autoref{fig:md_gilberto_lp}), the long non-island sentences scored by the GPT-2 model with a lower acceptability than by the human subjects, and on the right edge the blue line and orange line converge to the same low value. This does not happen in the BERT XXL  (\autoref{fig:md_bert_lp}) and GilBERTo models (\ref{fig:md_gilberto_lp}). This might be due to the GPT-2 model being more sensitive to more marginal sentences ending with a gap followed by a question mark, as in the long-distance dependency sentences tested for this type of island ("\textit{Che cosa pensi che Gianni avrebbe comprato \_?}", en: "\textit{What do you think Gianni has bought?}").
