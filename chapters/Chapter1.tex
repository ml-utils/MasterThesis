...
for adjunct islands, why the 6+ points drop in accuracy by BERT XXL (the model trained on the largest amount of data), compared to BERT and GilBERTo?

todo: series of token surprisals plots comparing directly BERT and BERT XXL


for subject islands, the progressive increase in accuracy, more or less correlates with amount of training data, from BERT (2B tokens), to BERT XXL (13B tokens), to GilBERTo (11B tokens).
for complex NP islands, the slight increase in performance (from 96 to 98 to 100) correlated with amount of training data
also for whether islands, the slight increase in performance, from 96 to 100. Performance seem to reache 100 somewhere bewttn 2B and 11B tokens.
...
In \autoref{fig:md_bert2b_adjunct_item_145_surprisals}, \ref{fig:md_bertxxl_adjunct_item_188_surprisals}, \ref{fig:md_gilberto_adjunct_item_185_surprisals} we can see token surprisals plots from BERT, BERT XXL, and GilBERTo on an adjunct island item. We can see that a prevailing factor influencing the models probabilities, and therefore the surprisals and the final sentence acceptability estimates, is how much a model has learned the probabilities of certain collocations. In particular we can see a difference between BERT and BERT XXL, seemingly due to the different amount of data they have been trained with (2B vs 13B tokens). 
...
In \autoref{fig:md_bert2b_adjunct_item_145_surprisals}, n the top sentence, we can see that BERT (the model trained with fewest data), has not learned a strong association between "\textit{emendamento}" ("\textit{emendament}") and "\textit{la camera boccerà}" ("\textit{the lower chamber will reject}"), and for this reason the word gets a surprisal spike highly influences the sentence final score. However, in the third sentence, we can see that the same word ("\textit{emendamento}") does not get the same spike, likely for the presence, far back at the beginning of the sentence, of the word "\textit{legge}" (\textit{law}), for which probably the model already has learned a strong association with "\textit{emendamento}".
