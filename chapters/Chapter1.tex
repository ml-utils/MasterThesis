...
Transformer-based language models were introduced in 2017 \citep{vaswani2017attention}, and since then they have revolutionized the NLP field, substantially advancing the state of the art in many benchmark and applications.
...
Determining which linguistic phenomena are actually learned by modern transformer-based language models has become increasingly important, since they are trained without any explicit linguistic supervision, and their internal representations are akin to "black boxes" very hard to interpret. Investigating the linguistic competence acquired by these models serves multiple purposes, like having a clear picture of their strengths and limitations, to identify areas for improving their robustness and their ability to generalize in a human-like way, and to accelerate work towards building general-purpose models for language understanding \citep{wilcox2018rnn, ettinger2020bert}.
...
% (we will discuss the respective island effect examples in Italian in \ref{sec:island_effects_in_italian}):
...
One of the main arguments against the existence of proper island "constraints" that render a sentence ungrammatical when violated, is that such ungrammaticality or unacceptability could arise instead just from  discourse-structural factors or simply from the increased \textit{processing difficulty} due to the sum of other factors present in alleged "island violations", like the fact of having a long-distance dependency combined with more marginal syntactic constructs \citep{sprouse2013experimental, wilcox2018rnn, ambridge2008island, hofmeister2010cognitive}. 
...
While English shows at least eight different types of island effects (whether islands, complex NP islands, subject islands, adjunct islands, relative-clause islands, sentential subject islands, coordinate structure constraint violations, and left branch extraction violations), Scandinavian languages (Swedish, Norwegian, Danish, and Icelandic), show almost no island effects\footnote{\citet{sprouse2013experimental} report that "Swedish is not bereft of apparent island effects. Rather it does not display island effects in all contexts where they are theoretically expected to appear (and as they do appear in English). For example, there are some unacceptable instances of extracting out of complex noun phrases, but others seem perfectly fine."} \citep{pearl2013syntactic}. Here are examples of the above mentioned eight types of syntactic islands in English, taken from \citet{sprouse2013experimental}:
...
Most island effects are present in Romance languages, with some exceptions, as some types of syntactic islands have effect in certain types of sentences, but not in others. This is the case of subject islands, which, in some configurations, have no effect in Italian, Spanish and Portuguese, while they are present in French \citep{sprouse2013experimental, pearl2013syntactic}. In a psycholinguistic study, \citet{sprouse2016experimental} found experimental evidence that subject islands with relative clause dependencies are not present in Italian. 
...
There is also a substantial varying degree of acceptability between the different types of islands. For instance, wh-islands are generally among the strongest sources of sentence unacceptability, while violations of complex NP islands with rc-dependencies tend to be more acceptabile \citep{pearl2013syntactic}. Because of this, it has been proposed in the literature a distinction between \textit{strong} and \textit{weak} islands \citep{sprouse2016experimental, szabolcsi2006strong}.
...
%As an introductory description of island effects, we list here examples of the four types of islands in Italian for which we conducted experiments. We give a fuller description of our test suite in  \ref{sec:testsuite_design}.
...
While most studies of island effects in English usually focus on syntactic islands built with wh-interrogatives, in his study Rizzi focused only on syntactic islands under relative clause dependencies, arguing that it was problematic to study them with wh-dependencies because they would require a double wh-word (as in \textit{*Chi ti domandi chi \_ ha incontrato \_?})(\textit{Who do you wonder who met?}), which itself renders a sentence unacceptable, resulting in a confound. 
...
\begin{exe}
	...
	\ex{\textsc{Complex NP Island} \\ {*Questo incarico, che non sapevo [la novità che avrebbero affidato \_ a te], ...} \\ "This task, which I didn't know the news that they may have assigned to you..."}\\
	\ex{\textsc{Subject island} \\ Questo autore, di cui so che [il primo libro \_] è stato pubblicato recentemente, ...\\ "This author, who I know that the first book was published recently, ..."} \\
\end{exe}
%
%\begin{exe}
%	\ex{\textsc{Wh-island}} 
...
This is because these are the most commonly studied types of wh-dependencies for syntactic islands, and also to match the examples in \citet{sprouse2016experimental} for comparison purposes. The type of wh-words (simple vs complex) is one of the factors that in principle could affect the processing of island effects and therefore their acceptability.

One aspect that influences the difference in sentence structure of syntactic islands in Italian, is the fact that Italian does not allow \textit{preposition stranding}, while English does. Preposition stranding is the possibility of having a preposition \textit{stranded} (separated from) its object. For example, in the English sentence \textit{I know \textbf{who} you bought the painting \textbf{from}}, "from" and "who" are stranded. Instead, Italian forces "\textit{pied-piping}" (another metaphorical technical term introduced by \citet{ross1967constraints}, like that of "islands", as mentioned in Sec. \ref{sec:islands_intro}), the fact that the prepositional phrase must be continuous (the wh-word \textit{brings along} the preposition): \textit{So \textbf{da chi} hai comprato il quadro} ("\textit{I know \textbf{from who} you bought the painting}").
...
A factorial experimental setup allows to quantify and tease out the effect of different factors that contribute to a phenomenon, in order to better control for confounds \citep{sprouse2016experimental, sprouse2012test}. Compared to a minimal pairs approach to language models assessment, a factorial test design is better suited to assess more complex syntactic phenomena like island effects \citep{warstadt2020blimp}.
...
A common use case is to have \textsc{items} (the equivalent of the sentence pairs in a minimal pairs approach) structured in a 2 × 2 paradigm, with four sentences in total, each across two binary \textsc{properties}. Each sentence exemplifies a \textsc{condition}, which is a particular state of the two properties. For instance, in the case of \citet{wilcox2018rnn}, in their factorial assessment of filler-gap dependencies, the two properties were the presence or absence of a filler, and the presence or absence of a gap. The same approach can be generalized to more complex paradigms, with more than two properties, each having more than two \textsc{levels} (a property with two levels is a binary property).
...
The factorial design allows to calculate the effect of the combination of the different properties, by making subtractions between the scores of the sentences of an item, because they differ minimally between each other (only on the basis of the controlled properties). This allows to factor out as many confound as possible, even unexpected ones, as long as they are evenly distributed across the sentences of an item \citep{sprouse2016experimental, hu2020systematic}.
...
As \citet{pearl2013syntactic} explain "translating each of these properties into separate factors, each with two levels (dependency GAP POSITION: matrix, embedded; STRUCTURE: non-island, island)," allows "to define island effects as a superadditive interaction of the two factors (..) in other words, an island effect [as captured by this definition] is the additional unacceptability that arises when the two factors are combined, above and beyond the independent contribution of each factor. Specifically, a syntactic island occurs when there is more unacceptability than what the EMBEDDED [long-distance] dependency and the presence of an ISLAND structure in the question contribute by themselves" \citep{pearl2013syntactic}. 

In an acceptability judgment experiment that employs a factorial definition of island effects, we isolate and quantify two effects that we want to factor out. One is the effect of dependency length, or \textit{length effect}, on acceptability, which we obtain by contrasting a \textit{baseline sentence} (which exemplifies the \textit{short-nonisland} condition), composed with a short WH-dependency (that is, the extraction is within the matrix clause itself), with a sentence (exemplifying the \textit{long-nonisland} condition) that contains a longer WH-dependency, where the extraction is from the embedded clause (while the filler is in the matrix clause). The other effect we want to isolate is the \textit{structure effect} of increase in processing difficulty due to the presence of a particular island structure. We isolate this by contrasting a sentence with an island structure (exemplifying the \textit{short-island} condition) with a sentence that does not contain an island structure (the baseline  \textit{short-nonisland} sentence). Finally, we can measure the total effect on acceptability of processing both long-distance WH-dependencies and island structures, by combining in a single sentence both properties (a sentence exemplifying the \textit{long-island} condition). We obtain the total effect from the difference in acceptability between the \textit{long-island} sentence and the baseline sentence \citep{sprouse2012test}. We can then calculate the \textit{island effect}, by subtracting from the total effect the length and structure effects.
...
\citet{hu2020systematic} used a factorial design in which each test items is composed of 4 minimal varying sentences, and obtains an overall percentage accuracy score by defining a success criterion (different for each test suite and syntactic phenomenon) on the score of a test suite item. %, without the need for statistical significance
...
\citet{wilcox2018rnn} circumvents the need for minimally different sentences, by scoring separately pairs of items with different constructs (i.e. one with and one without an island structure). These items are lexically similar between other, but differ more than a minimal pair (the item with the island construct is longer, with more content words). For both items, they measure the effect of the filler-gap dependency phenomena, whose effect is expected to drop in the presence of an island construct, which blocks it. If the drop in the effect is statistically significant, they can conclude that the model has "learned" the island constraint. \\
Both \citet{wilcox2018rnn, hu2020systematic} tested only conventional left-to-right unidirectional language models, they did not assess bi-directional language models like BERT.
...
While perplexity is not suitable as an absolute measure of a sentence grammaticality, it can be used as a \textit{relative} measure between a minimal pair of sentences, varying only for a property that makes one sentence grammatical and the other ungrammatical, as it has been done with the targeted syntactic evaluation (TSE) assessment paradigm.
...
The introduction of the targeted linguistic evaluation of language models, and the minimal pairs paradigm in particular, therefore serve also as a supplement to perplexity as a complementary measure that capture other aspects of a model performance. Additionally, a fine-grained linguistic assessment can provide more challenging tests, and it is also a tool that helps in the explainability of these models, since the test results from fine-grained linguistic assessment are more interpretable \citep{hu2020systematic}.
...
The targeted linguistic evaluation of deep neural language models trained on raw textual input started with the work of \citet{linzen2016assessing}, which used a corpus of naturally occurring sentences, extracted from the English Wikipedia, and tested the ability of Recurrent Neural Network models (RNNs), and in particular models based on long short-term memory units (LSTM). Linzen et al. used different testing approaches that will be developed in future work: (1) fine-tuning the models on a probing subject-verb-agreement prediction task (in which model saws the part of a sentence up to the word to predict), (2) fine-tuning the models on grammaticality judgments of full sentences, and (3) using the models' out-of-the box word prediction objective, seeing if assigns an higher probability to the correctly inflected word than the incorrect one \citet{linzen2016assessing}. 

\citet{gulordava2018colorless}, building on \citet{linzen2016assessing} evaluation paradigm, also used naturally occurring sentences on a subject-verb-agreement task, but added also nonce sentences that are grammatical but semantically meaningless, to tease out potential confounds due to semantic cues. 
...
On the other hand, the minimal pairs approach might be not be suitable or ideal when applied to more complex syntactic phenomena. Additionally, a model performance on a minimal pairs benchmark is not directly comparable to human scores. Instead, approaches like the one used in the Corpus of Linguistic Acceptability (CoLA), that require the model to be finetuned to return absolute acceptability judgments for each sentence in isolation, have the advantages of being directly comparable with both native speaker judgments and predictions made in the linguistic literature \citep{warstadt2019neural}. Targeted minimal pairs datasets might also be more labor-intensive to produce, when manually collected or constructed without the use of templates \citep{warstadt2020blimp}.	
...
A factorial experimental setup can be considered a generalization the minimal pairs approach. A factorial design allows to quantify and tease out the effect of different factors that contribute to a phenomenon, in order to better control for confounds \citep{sprouse2016experimental, sprouse2012test}. 
...
This descending curve in the per token surprisals in unigram LMs is intrinsic to the fact of being able to see only the left context of a token, and tends to hide the spikes in surprisals of individual tokens (due for instance to ungrammaticality or disfluency). For this reason, the per-token surprisals of bidirectional models like BERT, help to differentiate fluency from likeliness  \citep{salazar2020masked}.
...
, adjunct islands, and complex NP islands, but not subject islands. However, their study was primarily focused on Filler-Gap dependencies, and their experimental definition of island effects is based on a reduction in the expectations due to filler-gap dependencies, specifically the fact that, while in the absence of an island structure a model expects to find a gap if there is a filler present, this expectation is significantly reduced when there is also an island structure. This factorial experimental definition of island effects is different from the one used in psycholinguistic studies on human subjects \citep{sprouse2016experimental, bondevik2021variation}, in which island effects are measured as the residual acceptability difference between the baseline condition (absence of island structure, and absence of a filler-gap dependency), and island violation, after the contributing effects of the presence of an island structure and a filler-gap dependency have been subtracted away.
...
For cross-linguistic comparison, we reproduced the results on the English BLiMP benchmark reported by \citet{warstadt2020blimp, salazar2020masked} for the three island types it has in common with our test suites: wh-islands, adjuncts, and complex NP islands and we report our results in \ref{tab:accuracy_blimp}.
...
We report in \ref{tab:accuracy_it_data} the accuracy scores obtained in such a way, deriving minimal pairs from the factorial items.
%These accuracy scores of several Gpt and BERT-based models on an Italian island effects test suite developed for the present thesis. The minimal pairs are drawn from a factorial test design whose item are like \ref{adjunct_temporal_item_1}. 
Each acceptable sentence of an item like \ref{adjunct_temporal_item_1} is compared with the unacceptable sentence, and the score is considered accurate if the acceptable sentence receives an higher score than the unacceptable one.

For instance, on the first row of results, we see the scores for the whether island type, the acceptable Short-NonIsland sentence gets scored higher than the unacceptable sentence (Long-Island) 92\% of the times by the GPT-2 model with the LP sentence acceptability estimate, and 96\% of the time when the sentence score is normalized by length as in \citet{lau2020furiously}. The GilBERTo model (trained on 11B tokens) reaches the top overall average accuracy, with a score of 91.7\% which is similar to the one obtained with the factorial scoring.
...
%
%Although it's between different languages (English and Italian), the scores on \ref{adjunct_temporal_item_1} seem to be higher than the ones on the BLiMP benchmark in \ref{tab:accuracy_blimp}. 
...
\begin{table} \scriptsize 
	\begin{center}
		...
		\end{tabular}
		\caption{Accuracy results for Gpt-2 and BERT Italian models when using the sentences in a factorial design as minimal pairs. Evaluated on a test suite of 50 items per phenomenon developed for the present thesis.}
...
In this section we compare our results obtained from transformer-based language models with those from human subjects collected by  \citet{sprouse2016experimental}. We complement this with an analysis of the per-token surprisals given by the models for some of the items in our test suites.


With this qualitative analysis we try to explain the results in \ref{tab:accuracy_DD_it_data}. Specifically, the fact that (1) for adjunct islands, there is a 6+ points drop in accuracy by BERT XXL (the model trained on the largest amount of data), compared to BERT and GilBERTo. That (2) for subject islands, there is a progressive increase in accuracy, which more or less correlates with amount of training data, from BERT (2B tokens), to BERT XXL (13B tokens), to GilBERTo (11B tokens).

The fact that (3) complex NP islands, there is a slight increase in performance of BERT-based models (from 96\% to 98\% to 100\%) correlated with the amount of training data of each model. The fact that (4) GPT-2 shows a complete drop in performance (to 46\%) for this type of islands. The fact that (5) for whether islands, BERT (trained on 2B tokens) is the only model to score below 100\% accuracy, at 96\%, and therefore performance for this type of island seems to reach 100\% somewhere between 2B and 11B tokens.\\

In Fig. \ref{fig:sprouse_humans} we see the original plots from \citet{sprouse2016experimental}, on results collected from Italian subjects for wh-dependencies islands. Figures \ref{fig:md_gpt_penlp} to \ref{fig:md_gilberto_lp}\footnote{In these plots the scoring measures are indicated with the terminology by \citet{lau2020furiously}: LP stands for log probability, which for BERT models it's actually the pseudo-log-likelihood (PLL), while the sentence score normalized with a penalty term based on sentence length is indicated with PenLP.} show the plots we obtained for the same types of island constructs and wh-dependencies, with our test suite (that has a format similar to that of Sprouse et al.), on three Italian language models: the GPT-2-based GePpeTto \citep{de2020geppetto}\footnote{https://huggingface.co/LorenzoDeMattei/GePpeTto}, BERT (base-xxl version)\footnote{https://huggingface.co/dbmdz/bert-base-italian-xxl-cased} and the RoBERTa-based GilBERTo\footnote{https://huggingface.co/idb-ita/gilberto-uncased-from-camembert}. For comparison, we also show in Fig.\ref{fig:sprouse_gpt_penlp}-\ref{fig:sprouse_bert2b_lp} plots on the scores given by the three above language models on the original test suites by \citet{sprouse2016experimental}.

\begin{figure}[p]
	\centering
	\includegraphics[width=1\textwidth]{images/Chapter1/Plot_1_Sprouse_paper.png} 
	\caption{Interaction plots of island effects in humans from \citet{sprouse2016experimental}}
...
\begin{figure}[p]
	...
	\caption{GePpeTto - Interaction plots for each island type, with DD scores, on the test suites developed for the present thesis.}
...
\begin{figure}[p]
	...
	\caption{GilBERTo - Interaction plots for each island type, with DD scores, on the test suites developed for the present thesis.}
...
\begin{figure}[p]
	...
	\caption{BERT XXL (13B of training tokens) - Interaction plots for each island type, with DD scores, on the test suites developed for the present thesis.}
...
We can see that the plots for whether (top-left plot of each figure), adjunct (bottom-right), and subject islands (bottom-left) from the GPT-2 model (\ref{fig:md_gpt_penlp}) show some similarities with those obtained on human subjects (\ref{fig:sprouse_humans}), but also some significant differences.\\

%Looking at the plots in .., we preliminarly note .. resemblance with the plots for the average scores given by Italia speakers and reported by ..sprouse et al.
% todo: put here the factorial plots first, before the token surprisals plots

% first discuss the resemblance between the model plots and the human plots

% then discuss the comparison between the accuracy scores and the plots, and also compare with the token surprisals
% add estensive descriptive captions to each figure, also reprising the main text discussion
...
%
%
%In most other cases (island types: ..), there is no clear correlation between accuracy scores and plots resemblance to those of humans.  % note that these are average scores.. although error bars are shown, ..; so outliers might influence the average scores, while the accuracies.. are separate for each item.
...
% The score on the first row of \ref{tab:accuracy_blimp} are taken from \citet{warstadt2020blimp}


\subsection{Whether islands}
For whether islands, one difference between the plots from human scores and those from the models (\ref{fig:md_gpt_penlp}-\ref{fig:md_bert2b_lp}), is that for whether islands  the line connecting the \textsc{short-nonisland} and the \textsc{long-nonisland} goes upwards instead of downwards like for humans; that is, on average the language models give the \textsc{long-nonisland} a higher acceptability estimation score (the pseudo-log-likelihood, PLL), than the \textsc{short-nonisland} (and this trend remains also when the models score the original test suite administerd by Sprouse to humans, diplayed in \ref{fig:sprouse_gpt_penlp}-\ref{fig:sprouse_bert2b_lp}).

To explain why, we can have a look at \ref{fig:md_bert_whether_item_158_surprisals}, which shows the surprisals of each token in one of the whether islands items. Plots showin the surprisal for each token in a sentence are a kind of "zoom" into the sentence acceptability estimates, since, in the case of BERT-based models, the sentence score (the pseudo-log-probability) is just the (negative) sum of the token surprisals. 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for the four sentences of one of the whether island items, as scored by GilBERTo.} 
...
In \ref{fig:md_bert_whether_item_158_surprisals}, we see that the \textsc{short-nonisland} sentence ("\textit{Chi pensa che io abbia studiato la teoria?}") has the additional tokens "\textit{la teoria}", which significantly adds to the total sum of surprisal. The same phenomenon can be see in \ref{fig:md_gilberto_whether_item_152_surprisals}, where the phrase "\textit{vinto il premio}" (found in the \textsc{short-nonisland}) sums up to slightly more surprisal than the word "\textit{vinto}" alone (found in the \textsc{long-nonisland}). However, in general we can't just assume that a longer sentence will get a lower acceptability scores, because having more content words can give more semantic cues, making it easier to predict each of them when it's masked during the scoring, lowering the surprisals (as can be seen in \ref{fig:md_gilberto_adjunct_item_195_surprisals}, discussed in the next section).\\

% ..

% \subsubsection{Whether islands}

%From \ref{fig:wh_whether} for whether islands with wh-dependencies, we can see that the GTP-2 model, compared with the results from humans gives higher (more acceptable) scores for all sentence types. This difference is more pronounced in the scores performed by Gpt on the original test suite from \citet{sprouse2016experimental} (middle image). 
...
Back to the question of why BERT is the only model not to reach 100\% accuracy on whether islands, there seems to be no clear explanation for this fact looking at the token surprisals of the same item across all the models, as shown in \ref{fig:md_bert2b_whether_item_180_surprisals}-\ref{fig:md_gpt_whether_item_135_surprisals}.
One thing we notice is that the baseline acceptable sentence (\textsc{short-nonisland}) ("\textit{Chi pensa che io abbia aumentato l'affitto?}") ("\textit{Who thinks that I have raised the rent?}") gets a lower acceptability in BERT XXL (\ref{fig:md_bertxxl_whether_item_156_surprisals}) than BERT (\ref{fig:md_bert2b_whether_item_180_surprisals}). The trend of the surprisal plots seems to be almost the same between BERT and BERT XXL, with a few difference. We can notice the strange phenomenon that BERT XXL gets a high surprisal (much higher than BERT) on the words "\textit{affitto}" ("\textit{rent}") and "\textit{io}" ("\textit{I}"). Again this seems another case of semantic phenomena having a prevailing confounding effect, that could mask the effect of syntactic ones.

% sentences on Chi pensa che io abbia aumentato l'affitto?
...
\begin{figure}[p]
	...
	\caption{Token surprisals for a whether island item, as scored by GilBERTo.} 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for a whether island item, as scored by the Italian BERT.} 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for a whether island item, as scored by the Italian BERT XXL.} 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for a whether island item, as scored by GePpeTto.} 
...
\begin{figure}[H]
	...
	\caption{Token surprisals for the four sentences of one of the adjunct island items, as scored by GilBERTo.} 
	\label{fig:md_gilberto_adjunct_item_195_surprisals} 
	\medskip
	\small
\end{figure}

Also in \ref{fig:md_gilberto_adjunct_item_195_surprisals} (an adjunct island item scored by GilBERTo), we can notice what seems to be causing the \textsc{short-island} sentences to have a higher acceptability estimate than \textsc{short-nonisland} ones, and therefore the island and non island lines to "cross" in \ref{fig:md_gilberto_lp} (plot at the bottom-right). The \textsc{short-nonisland} too has the phrase "\textit{il sindacato indirà lo sciopero}", but it has also additional content words that give semantic cues and therefore further lower the surprisals, with the phrase "\textit{chiuderà i battenti}".

GPT-2 seem to have learned adjunct island constraints more robustly than the two BERT-based models, as mentioned above. Looking at \ref{fig:md_gpt_penlp} it seems this is due to the higher gap in acceptability given by GPT-2 than BERT and GilBERTo (\ref{fig:md_gilberto_lp}). In the case of GPT-2, the gap it's almost 1 standard deviation (these are normalized z-scores), while it's about half that for the BERT-based modes (\ref{fig:md_gilberto_lp}).\\
...
%
%
%In \ref{fig:wh_adjunct} the slope of both lines on the new test suite (right image) is similar to the human scores (left image), althoughg with a lower average acceptability for the Long-NonIsland sentences, which results in a downward steeper line. 
%
...
We can complement our analysis so far, with a comparison of the token surprisals for the same sentences across the four models we tested. In \ref{fig:md_bert2b_adjunct_item_145_surprisals}, \ref{fig:md_bertxxl_adjunct_item_188_surprisals}, \ref{fig:md_gilberto_adjunct_item_185_surprisals}, \ref{fig:md_gpt_adjunct_item_163_surprisals} we can see token surprisals plots from BERT, BERT XXL, GilBERTo and GePpeTto on an adjunct island item. 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for an adjunct island item, as scored by the Italian BERT.} 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for an adjunct island item, as scored by the Italian BERT XXL.} 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for an adjunct island item, as scored by GilBERTo.} 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for an adjunct island item, as scored by GePpeTto.} 
	\label{fig:md_gpt_adjunct_item_163_surprisals} 
	\medskip
\end{figure}

In \ref{fig:md_bert2b_adjunct_item_145_surprisals}, in the top sentence, we can see that BERT (the model trained with the least data), has not learned a strong association between "\textit{emendamento}" ("\textit{emendament}") and "\textit{la camera boccerà}" ("\textit{the lower chamber will reject}"), and for this reason the word gets a surprisal spike that highly influences the final sentence score. However, in the third sentence, we can see that the same word ("\textit{emendamento}") does not get the same spike, likely for the presence, far back at the beginning of the sentence, of the word "\textit{legge}" (\textit{law}), for which probably the model already has learned a strong association with "\textit{emendamento}".

Looking in \ref{fig:md_bertxxl_adjunct_item_188_surprisals} at the plots for the same sentences from BERT XXL (trained with billions more data), we can see that the spikes in the first sentence for \textit{"camera"} ("\textit{lower chamber}") and  "\textit{emendamento}" ("\textit{emendament}"), have been much lowered compared to the previous model, likely because after 13B tokens of training data BERT has finally learned the association between these two words.
...
Conversely from the adjuncts islands, GPT-2 seems to struggle with complex NP islands (46\% accuracy in \ref{tab:accuracy_DD_it_data}), compared to BERT (100\% accuracy) and GilBERTo (98\% accuracy). This is also reflected in the factorial plots showing the average sentence acceptability scores: the one for BERT (\ref{fig:md_bert_lp}) and GilBERTo (\ref{fig:md_gilberto_lp}) have slopes similar to the plots from human scores (\ref{fig:sprouse_humans}), while GPT-2 stands out as different (\ref{fig:md_gpt_penlp}).
...
%In the middle and right images on \ref{fig:wh_complex}, we see that, for complex NP items, the long non-island sentences (blue line, right edge) on average get scored by the GPT-2 model with a lower acceptability than by the human subjects, shown in the left image. 

% (\ref{fig:md_gpt_penlp}) BERT XXL (\ref{fig:md_bert_lp}) \ref{fig:md_gilberto_lp}

We see that, for complex NP items (top-right plot of each figure from \ref{fig:sprouse_humans} to \ref{fig:md_bert2b_lp}), the long non-island sentences scored by the GPT-2 model with a lower acceptability than by the human subjects, and on the right edge the blue line and orange line converge to the same low value. This does not happen in the BERT XXL  (\ref{fig:md_bert_lp}) and GilBERTo models (\ref{fig:md_gilberto_lp}). This might be due to the GPT-2 model being more sensitive to more marginal sentences ending with a gap followed by a question mark, as in the long-distance dependency sentences tested for this type of island ("\textit{Che cosa pensi che Gianni avrebbe comprato \_?}", en: "\textit{What do you think Gianni has bought?}").\\
...
Back to the question of why BERT XXL achieves 100\% accuracy in complex NP islands, going 4 points above BERT, we can have a look at the token surprisals plots in \ref{fig:md_bertxxl_complex_np_item_32_surprisals} and \ref{fig:md_bert2b_complex_np_item_173_surprisals} for a sentence that BERT XXL scores correctly (DD score = 17.75) and BERT incorrectly (-0.21). 

We observe that the difference in the final DD score, seems largely driven by the better acceptability that BERT XXL gives to the \textsc{long-nonisland} sentence (second from the top in the plots), which results in a much lower expected length effect  (7.95 vs 18.86) in the final factorial calculation. Looking at the sentence in question \textit{"Cosa hai rivelato che il consiglio di amministrazione avrebbe redatto?"} (\textit{"What did you reveal that the board of directors had drafted?"}) We see that BERT assigns a larger surprisal value to the region "\textit{Cosa hai rivelato}" ("\textit{What did you reveal}"), and this could in fact be due to a syntactic phenomena, with BERT XLL having learned the less common construct that this phrase introduces here. We also see that BERT gives a much larger surprisal to the word "\textit{amministrazione}" (literally "\textit{administration}", with stands for "\textit{directors}" in the multi-word expression (MWE) equivalent to "\textit{board of directors}"), and this could instead be due to BERT not having learned yet (even after 2B tokens of pretraining data) the collocation "\textit{consiglio di amministrazione}".
...
\begin{figure}[p]
	...
	\caption{Token surprisals for a complex NP island item, as scored by GilBERTo.} 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for a complex NP island item, as scored by the Italian BERT XXL.} 
...
\begin{figure}[p]
	...
	\caption{Token surprisals for a complex NP island item, as scored by the Italian BERT.} 
...
Back to the question of explaining the correlation between the amount of model training data and the performance of these models on subject islands, we can have a look at the token surprisal plots for the same test item across the four models (\ref{fig:md_bert2b_subject_item_185_surprisals}-\ref{fig:md_gpt_subject_item_171_surprisals}). In \ref{fig:md_bertxxl_subject_item_167_surprisals}, we see that BERT XXL gives a better acceptability to the baseline sentence than the BERT model (\ref{fig:md_bertxxl_subject_item_167_surprisals}), but this seems to be due again to lexical reasons, because BERT XXL gives much smaller surprisals to the words "\textit{foto}" ("\textit{photo}") and to the OOV word "\textit{scandalizzato}" ("\textit{scandalized}"). We also observe that in the two bottom sentences ("\textit{Di chi pensi che la foto dell'artista abbia suscitato l'indignazione?}", "\textit{Di chi pensi che la foto abbia suscitato l'indignazione del pubblico?}") ("\textit{of whom do you think that the artist picture caused the outrage?}", "\textit{of whom do you think that the picture cause the outrage of the audience?}") we notice that BERT, compared to BERT XXL, gives very high surprisal to an  apostrophe punctuation marking the determiner elision and followed by a OOV word ("\textit{l'indignazione}") ("\textit{the outrage}").\\


\begin{figure}[p]
	\centering
	\includegraphics[width=1\textwidth]{images/Chapter1/token_surpisals/Madeddu_dbmdz_bert-base-italian-cased_wh_subject_islands_item_185.png} 
	\caption{Token surprisals for an subject island item, as scored by the Italian BERT.} 
	\label{fig:md_bert2b_subject_item_185_surprisals} 
	\medskip
\end{figure}

\begin{figure}[p]
	\centering
	\includegraphics[width=1\textwidth]{images/Chapter1/token_surpisals/Madeddu_dbmdz_bert-base-italian-xxl-cased_wh_subject_islands_item_167.png} 
	\caption{Token surprisals for an subject island item, as scored by the Italian BERT XXL.} 
	\label{fig:md_bertxxl_subject_item_167_surprisals} 
	\medskip
\end{figure}

\begin{figure}[p]
	\centering
	\includegraphics[width=1\textwidth]{images/Chapter1/token_surpisals/Madeddu_idb-ita_gilberto-uncased-from-camembert_wh_subject_islands_item_159.png} 
	\caption{Token surprisals for an subject island item, as scored by GilBERTo.} 
	\label{fig:md_gilberto_subject_item_159_surprisals} 
	\medskip
\end{figure}

\begin{figure}[p]
	\centering
	\includegraphics[width=1\textwidth]{images/Chapter1/token_surpisals/Madeddu_LorenzoDeMattei_GePpeTto_wh_subject_islands_item_171.png} 
	\caption{Token surprisals for a subject island item, as scored by GePpeTto.} 
	\label{fig:md_gpt_subject_item_171_surprisals} 
	\medskip
\end{figure}

\subsection{Other observations}

We found confirmation of what found by \citet{salazar2020masked}, that for bidirectional models like BERT, it's better not to normalize by sentence length the sentence score (the pseudo-log-likelihood), contrary to what proposed by \citet{lau2020furiously}. %In addition to the explanation for this given by \citet{salazar2020masked}, which analyzed the difference in numerical properties inherent in unidirectional vs bidirectional models, we find reasons for this in the fact 
As observed by \citet{salazar2020masked}, a sentence final acceptability score, as obtained by the sum of the surprisal in a bidirectional LM, has a "flat" trend, independent to its length, and it is determined instead by surprisal spikes of a few tokens, often for semantic reasons.

We can see the degrading effect in performance, due to normalizing by sentence length, comparing the plots for BERT XXL without normalization (\ref{fig:md_bert_lp}) and with normalization (%\ref{A-fig:md_bert_penlp} 
Figure A.1 in the Appendix). This is shown as the divergence of the models plots, from those from the human scores, are exacertbated when using sentence length normalization. The two lines (connecting the average values of each sentence type) cross for complex NP islands, they cross even more for subject islands, and are parallel (indicating absence of an island effect) for adjunct islands. Similar divergences from the human plots, when using the length-normalized PenLP score, also happen with GilBERTo (Figure A.3 in the Appendix vs \ref{fig:md_gilberto_lp}). 

Conversely, for unidirectional models like GPT-2, the length-based normalization of the sentence score has a beneficial dampening effect to the model score numerical properties, as noted by \citet{salazar2020masked}. And this is also reflected in the plots for GePpeTto, with a divergence from human plots when using the non ideal, unnormalized, LP score (compare \ref{fig:md_gpt_penlp} with Figure A.4 in the Appendix). However, we were not able to reproduce what described by \citet{salazar2020masked} in their analysis of the numerical properties of unidirectional models, as compared to bidirectional ones. Specifically, our GPT-2 token surprisals plots do not show a descending curve (with high surprisals for the first tokens in the sentence, that diminish as the model has access to more context). Instead, the trend in this plot seems to be also "flat" as those for the BERT-based models. This is possibly due to a methodological difference in the way we obtained the token surprisals from the GPT-2 output.


In general, we found that the token surpisal plots for GPT-2 to be much less clearly interpretable than those from the BERT-based models. One thing we were able to observe from the token surprisals from GPT-2, were surprisal spikes in correspondence of sub-units of OOV words.\\

In \ref{fig:md_gpt_subject_item_152_surprisals} we observe the non-optimal sub-word units splits performed by the tokenizer used by GPT-2: the out-of-vocabulary (OOV) word \textit{"impoverito"} (\textit{"impoverished"}) has been split into \textit{imp-ov-erito}, which clearly cannot take advantage of the semantically informative root \textit{"pover-"} ("\textit{poor}"). \citet{bostrom2020byte} found that sub-word splitting algorithms like BPE % does Gpt/geppetto use BPE?
...
%
%We choose to focus on this combination because it seems to produce in general more accurate results as seen in \ref{tab:accuracy_it_data}. 
%
...
%\end{figure}
%
%In \ref{fig:wh_subject}, the middle image of Gpt scores on Sprouse data shows significantly lower scores (compared to the human scores on the left image) for the sentences with an island structure (dotted line). % check the sorting of sentences to see why). 
...
% For future work we would like to control for more confounds, splitting the testsuites in smaller ones (of about 20 items each as in Hu et al and ..), each exemplyfying only one paradigm (eg. only conditional ajduncts or only temporal adjuncts), 
...
%In \ref{fig:bert_penlp_l_sprouse} we see the plots of the scores from the BERT model, with the PenLP-L sentence acceptability estimate (using the logistic function rather than the softmax, as described in \ref{sec:softmax}). Interestingly, it seems that this combination of BERT with the logistic function makes a clear separation between unacceptable sentences (the Long-NonIsland type) and acceptable sentence types, with minimal, if any, difference in score between acceptable sentences.
...
%In \ref{fig:wh_complex} we see that the non-island line (the line connecting the two acceptability scores for short and long distance dependency sentences without an island structure) is significantly lower in new data (todo: see constructs that increase the score of both long and short)
%
...
%Doing some variations experiments (see  \ref{tab:compare1}), we found that by replacing the main clause verbs with "intuire che"/"avere l'intuizione che" ("to sense that" / "to have the intuition that") or "avvertire che"/"avere il sentore che" ("to feel that" / "to have an inkling that") or "percepire che"/"avere la percezione che" ("feel that"/"to have the feeling that"), the island restriction violation seem to have no effect, and the Long-Island sentence becomes more acceptable than the same sentence without the island structure (Long-NonIsland).
%
...
% follow-up experiments if there were time within this thesis, or as additions after the final upload on ETD, to be shared directly with the supervisors, and for the printed form, and to be discussed during the graduation presentation.

In future work, we plan to do develop a revised test suite to control for more confounds. Among the things we plan to do, are: try to strictly balance the sentences in each item to use the same lexical content. Control for the increased processing complexity due to interrogative forms, by duplicate the test suites with versions in the affirmative, as in \citet{wilcox2018rnn}, and in the interrogative. Splitting the test suite for adjunct islands into temporal adjuncts and conditional adjuncts, and potentially other types of adjunct islands. Control for OOV words, possibly creating a duplicate for each test suite, one with and one without OOV or more rare words. Controlling for using simple vs complex wh-phrases as fillers in wh-dependencies. Controlling for less common verb forms, e.g. using the indicative mood instead of the subjunctive mood. Controlling for the effects of using personal pronouns and proper names (e.g. \textit{Gianni, Luisa}) instead of common nouns (e.g. \textit{the student}, \textit{the doctor}, etc.). Controlling for the effect of using, within a sentence, words with strong association (e.g. \textit{the student reads the book}, \textit{the policeman arrested the burglar}, etc.), vs unexpected associations.

We plan to expand our test suites to test also across rc-dependencies, as in \citep{sprouse2016experimental}, and compare the differences across the same island construct types tested here with wh-dependencies.

We plan to translate into Italian the filler-gap and island effects test items from \citet{wilcox2018rnn}, and try their approach of assessing island effects as a reduction in expected gaps in the presence of a filler. We plan to compare their approach with the one by \citet{sprouse2016experimental}, that we adopted in the present thesis.

Finally, we plan to translate and adapt our test suites into English, and to compare the results with those from English speakers collected by \citet{sprouse2016experimental}, and to compare them also with the reported results on English island effects on the BLiMP test suites, and those from \cite{wilcox2018rnn}.

% Revising our approach to extract per-token surprisals from GPT-2, to reproduce the descending curve of surprisals along the sentence length, as observed by \citet{salazar2020masked}.
...
% testsuites from psychonlinguistic studies need to be adapted considerably when applied to neural language models, which seems to be much more sensitive to confounding semantic factors than syntactic ones when evaluating a sentence probability or pseudo-likelihood. % (which correlate with human processing difficulty).
