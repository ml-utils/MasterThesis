...
Consider the following examples (taken from \citet{sprouse2016experimental}), in which (\ref{no_filler_gap_1}) is a plain sentence with no “extraction”\footnote{The use of the term \textit{extraction} to refer to filler-gap dependencies evokes the idea of movement formulated in transformational grammars within the generative linguistic paradigm, as if the filler is moved or extracted from the canonical argument position \citep{sprouse2013experimental}.}, and (\ref{filler_gap_1}) contains a filler-gap dependency (the gaps are marked with underscores, and the fillers in bold) :
...
A factorial design allows to quantify and tease out the effect of different factors that contribute to a phenomenon, in order to better control confounds \citep{sprouse2016experimental}. Compared to a minimal pairs approach to language models assessment, a factorial test design is better suited to assess more complex syntactic phenomena like island effects \cite{warstadt2020blimp}.

In out case, for each of the four island effect types that we want to test, we built 50 \textsc{items}, each containing 4 sentences (one for \textsc{condition}) covering a 2x2 factorial design crossing two \textsc{factors}: presence or absence of an island structure (\textsc{structure} factor), and presence of a short or long-distance dependency (\textsc{gap-position} factor).
...
\begin{itemize}  
	\item \textsc{lenght effect} = $PLL_a - PLL_b$
	\item \textsc{structure effect} = $PLL_a - PLL_c$
	\item \textsc{total effect} = $PLL_a - PLL_d$
	\item \textsc{island effect} = $\textsc{total effect} - (\textsc{lenght effect} + \textsc{structure effect})$
\end{itemize}  

The \textsc{lenght effect} should capture the decrease in sentence acceptability due to the increase of the dependency distance from short to a long filler-gap dependency. The \textsc{structure effect} should capture the effect of adding an island structure. \textsc{total effect} should capture the total effect. Subtracting the first two effects from the total effect should isolate the \textsc{island effect} alone, without consider the independent sentence processing cost due to the other two factors. We conclude that a model has 'learned' the island effect displayed in an item, if the island effect score on that item is greater than zero. 

This is factorial score is used by \citet{sprouse2016experimental} to assess island effect in human subjects, equivalent to a differences-in-differences (DD) score \citep{maxwell2003designing}. Analogous factorial score have been used for language models by \citet{wilcox2018rnn} and \citet{hu2020systematic}.
