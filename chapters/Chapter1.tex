...
In the middle and right images on \autoref{fig:wh_complex}, we see that, for complex np items, the long non-island sentences on average get scored by the Gpt-2 model with lower acceptability scores than by the human subjects (left image). This seems to be due to the long distance dependency effect, that gets exacerbated as a sentence increases in lenght (in these testsuites, the complex np islands examples have longer sentences).
...
% table..
Variations on  \autoref{tab:compare1}

\begin{table} \scriptsize 
	\begin{center}
		\begin{tabular}
			{p{0.3\linewidth} p{0.08\linewidth} p{0.3\linewidth} p{0.08\linewidth} p{0.08\linewidth}|} \\
			\multicolumn{2}{c}{\textbf{Long non-island}} & \multicolumn{2}{c}{\textbf{Long island}}  & \multicolumn{1}{c}{\textbf{Diff}} \\
