...
In building test suite for our experiments, we largely followed the format of the target material in Italian that was administered by \citet{sprouse2016experimental} to human subjects. However, we increased the number of items per phenomenon from 8 to 50, and we made some adjustments (discussed in the following paragraphs) to reduce potential confounds in administering the material to Language Models (LMs), due to the way in which these models estimate likelihood scores for sentences.

For each of the four island effect types, we built 50 items, each containing 4 sentences covering a 2x2 factorial design crossing two factors: presence or absence of an island structure, and presence of a short or long-distance dependency.

Consider the item in the following example (\ref{adjunct_island_2_it}), in which condition a. displays a short-distance dependency and no island structure; condition b. displays a long-distance dependency and no island structure; condition c. displays a short-distance dependency and an adjunct island structure; and condition d. displays a long-distance dependency crossing an adjunct island structure:

\begin{exe}
	\ex{\textsc{Adjunct island}}\label{adjunct_island_2_it}
	\begin{xlist}
		\ex\gll Chi dice che {l'imprenditore} ha raccolto i fondi sufficienti? \\
		Who say that {the businessman} have raised the funds sufficient\\
		\glt `Who says that the businessman raised sufficient funds?'
		
		\ex\gll {Che cosa} dici che {l'imprenditore} ha raccolto \_?\\
		What say that {the businessman} have raised\\
		\glt `What do you say that the businessman raised?'
		
		\ex\gll Chi ha avviato {l’attività} [quando ha raccolto i fondi sufficienti]?\\
		...
		\ex\gll {*Che cosa} {l'imprenditore} ha avviato {l’attività} [quando ha raccolto \_]?\\
...
%Chi dice che Lucia pubblicherà il libro?
%Che cosa dici che Lucia pubblicherà \_?	
%Che cosa Lucia scriverà una recensione se l'editore pubblicherà \_?	
%Chi scriverà una recensione se l'editore pubblicherà il libro?	

%Chi pensa che io abbia scritto la lettera? [Non-island, short-distance-dependency]
%Cosa pensi che io abbia scritto \_? [Non-island, long dependency]
%Chi si domanda se io abbia scritto la lettera?	
%Cosa ti domandi se io abbia scritto \_?		
