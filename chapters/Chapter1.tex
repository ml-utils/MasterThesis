...
% "A language model (LM) defines a probability distribution over sequences of words. "  \citep{marvin2018targeted}
While linguists are usually concerned with grammaticality or \textit{acceptability} judgments when assessing linguistic expressions, computational language processing has traditionally been more concerned with \textit{likelihood}, i.e. the probability of a sentence being produced or encountered \citep{lau2020furiously}. But the concepts of grammaticality and probability, while related and partially overlapping, do not coincide. A fundamental research question is to determine how these two concepts are related \citep{lau2020furiously}. %computational language models learn a probability distribution of words and expressions. 
%"The question of whether and how these properties are related is a fundamental one" \citep{lau2020furiously}
In other words, while computational language models learn a probability distribution of words and expressions, this does not corresponds exactly to a grammaticality or acceptability judgment \citep{marvin2018targeted}.

The concept of acceptability broader than that of grammaticality. Grammaticality usually refers to syntactic acceptability (well-formedness), but an acceptability in general includes also semantic, pragmatic, and even non-linguistic factors (e.g. sentence length) \citep{lau2020furiously}. Additionally acceptability is usually a graded judgment on a sentence, while grammaticality is a binary one (a sentence can be either grammatical or not).
...
require the models to be finetuned on an additional dataset on an acceptability detection task.

\citet{warstadt2019neural} introduced the Corpus of Linguistic Acceptability (CoLA), a collection of about 10k of sentences labeled with acceptability judgments in the linguistic literature. It was developed to be used with the absolute acceptability judgments approach, and required the models to be fine-tuned on an sentence acceptability task with a subset of the data. 
...
A successor of CoLA was the BLiMP dataset, which was developed for the minimal pairs paradigm instead. \citep{warstadt2020blimp}.
