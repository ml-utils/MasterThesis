...
In this section we compare our results obtained from transformer-based language models with those obtained by \citet{sprouse2016experimental} on human subjects. The
...
in the two datasets the stimuli are similar, but there are differences (for instance, out adjunct islands teststuite includes temporals adjunts, while in the Sproure one includes only conditional adjuncts, as noted in section ..).

We can see that the plots for whether, adjunct, and subject islands are quite similar with those obtainad on human subjects, with the difference that for adjunct islands, the SHORT-ISLAND sentence gets a higher acceptability score from the transformer model (this might be due to the confound deriving from the format of this sentence, as discussed in section ..).
