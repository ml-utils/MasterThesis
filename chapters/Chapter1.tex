...
In \autoref{fig:bert_penlp_l_sprouse} we see the plots of the scores from the BERT model, the logistic function (rather than the softmax) applied to the outputs, and the PenLP sentence acceptability approximation. Interestingly, it seems that this combination of BERT with the logistic function distinguishes clearly between unacceptable (long-nonisland) and acceptable sentence types, while scores all the acceptable sentences the same.

If we compare this with the accuracy scores from this model (BERT, logistic function, PenLP) in table ..3.1, we see that they are among the best in about half the cases, and quite low for the long-nonisland sentences of all four island phenomena (58\%, 64\%, 74\%, 58\% accuracy), and for the short-nonisland of subject islands (28\% accuracy) 
