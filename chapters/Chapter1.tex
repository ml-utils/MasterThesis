...
In building test suite for our experiments, we largely followed the factorial design of the target material in Italian that was administered by \citet{sprouse2016experimental} to human subjects. However, we increased the number of items per phenomenon from 8 to 50, and we made some adjustments (discussed in the following paragraphs) to reduce potential confounds in administering the material to Language Models (LMs), due to the way in which these models estimate likelihood scores for sentences.
...
Differences with the target material in \citet{sprouse2016experimental}
